...
export function App() {
  ...
  return <main className={`workbench ${leftOpen ? '' : 'left-collapsed'} ${rightOpen ? '' : 'right-collapsed'} ${bottomOpen ? '' : 'bottom-collapsed'}`}>
    <header className="topbar">
      ...
      <div className="transform-tools" aria-label="Transform mode">{(['translate', 'rotate', 'scale'] as const).map((mode) => <button key={mode} data-testid={`transform-mode-${mode}`} title={mode} className={transformMode === mode ? 'active' : ''} onClick={() => setTransformMode(mode)}>{mode === 'translate' ? '↗ Move' : mode === 'rotate' ? '↻ Rotate' : '↔ Scale'}</button>)}</div>
